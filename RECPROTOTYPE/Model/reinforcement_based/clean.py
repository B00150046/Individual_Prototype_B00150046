...
# ---------- load data once ----------
ratings = pd.read_csv("data/book_users.csv")
...
def recommend_similar(book_title, df, X, movie_mapper, movie_inv_mapper, k=5, return_ids=False, verbose=True):
    ...
    neighbor_ids = [int(movie_inv_mapper[i]) for i in indices.flatten()]
    ...
    return neighbor_ids if return_ids else recommendations
